...
Половину настроек macOS в «Системных настройках» не найти, но они есть — через `defaults write` в терминале. Удобно при переезде на новый Mac: один скрипт, и Finder, Dock, скриншоты, клавиатура ведут себя как надо.

Ниже — мой скрипт с короткими комментариями. Справка по всем ключам: [macos-defaults.com](https://macos-defaults.com/).
...
**Трёхпальцевое перетаскивание** — перетаскиваю окно или выделенный текст тремя пальцами, без drag lock. Без этого надо сначала «нажать», а потом тянуть, мне это всегда мешало.
...
Скрипт ниже собирает все настройки выше. Запуск: `./macos-defaults-examples.sh` (сначала `chmod +x macos-defaults-examples.sh`). Или просто копируйте нужные блоки к себе.
...
После — перезапустить Finder и Dock:

```bash
killall Finder Dock
```

Некоторые настройки (трекпад, например) подхватятся только после релогина или перезагрузки. Полный список ключей: [macos-defaults.com](https://macos-defaults.com/).
